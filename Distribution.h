...
class diskPosition_distribution : public distribution<point> {
private:
    double       oriX, oriY, oriZ, rad;
    const double twopi = 2.0 * std::acos(-1.0);
public:
    diskPosition_distribution( std::string label, double a, double b, double c, double d)
    : distribution(label), oriX(a), oriY(b), oriZ(c), rad(d) {};
    ~diskPosition_distribution() {};
