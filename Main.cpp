...
int main() {
        ...
        for ( auto d : input_distributions ) {
            ...
            // integer-valued distributions
                ...
                if ( type == "delta" ) {
                    ...
                    Dist = std::make_shared< diskPosition_distribution > ( name, xOri, yOri, zOri, rad);
                }
    ...
    pugi::xml_node input_numHistory_nsamples = input_numHistory.child("histories");
    int nsamples = input_numHistory_nsamples.attribute("end").as_int();
    int cellNum;
        ...
        while( ! pbank.empty() ){
            ...
            while ( p.alive() ) {
                ...
                if(impNow > 0.0){
                    ...
                    double sigma = cells[cellNum]->macro_xs();
                    ...
                    cells[cellNum]->moveParticle( &p, distance  );
                    numMove = numMove + 1;
                    ...
                    if ( distance == dist_surface ) {
                        ...
                        if(impNow > impNew){
                            ...
                            //also reduce their weight
                            for(int jentry = 0 ; jentry< (numSplit-1) ; jentry++){
                                particle q(  p.pos(), p.dir()  );      //create new particles with weight 1
